...
import Defered from "./Defered.js";
...
export default class MarlinServer extends EventEmitter {
  ...
  async ready() {
    ...
    let serialPort = this.options.serialport || null;
  ...
  resetQueue() {
    ...
    while ((promise = this.promiseQueue.shift()))
      promise.reject(
        new Error(
          `Command "${promise.command}" canceled due to connection opening`
        )
      );
    // this.promiseQueue = []
  }
  ...
  dataReceived(data) {
    ...
    {
      var [$, temp_bed, temp_bed_target] =
        data.match(/B:(\d+\.?\d*)\s*\/(\d+\.?\d*)/) || [];
      var [$, temp_extruder, temp_extruder_target] =
        data.match(/T:(\d+\.?\d*)\s*\/(\d+\.?\d*)/) || [];
    ...
    if (
      this.marlinOptions.START_TOKEN &&
      data.startsWith(this.marlinOptions.START_TOKEN)
    ) {
      if (this._started) {
